...
export function AppShell() {
  ...
  const [sidebarOpen, setSidebarOpen] = useState(
    () => !initialNavigation.sidebarCollapsed,
  );
  ...
  const [activeTopPanel, setActiveTopPanel] = useState<
    "agents" | "branches" | "system" | "tools" | "session" | null
  >(null);
  ...
  useEffect(() => {
    ...
    // A subagent viewed inside its family's tab mirrors its own id, so a
    // reload reopens the subagent view; otherwise the tab's session mirrors.
    const search =
      selectedSession?.relation?.kind === "subagent" &&
      activeTab?.sessionId !== undefined &&
      activeSessionFamily?.root.id === activeTab.sessionId
        ? `?session=${encodeURIComponent(selectedSession.id)}`
        : buildUrlSearch(tabState.activeId, tabState.tabs);
    const currentSearch =
  ...
  const handleSelectSession = useCallback(
    (
      ...
      // Subagent sessions navigate inside their family's tab — selecting one
      // never creates a tab; it just activates the family root's tab (opening
      // one for the root if the family has no tab yet).
      ...
      setTabState((current) => {
        ...
        if (existing) return activateSessionTab(current, existing.id);
        if (session.relation?.kind === "subagent") {
          const rootId =
            getSessionFamily(sessionsWithSelection, session.id)?.root.id ??
            session.relation.parentSessionId;
          const rootTab = current.tabs.find((t) => t.sessionId === rootId);
          return rootTab
            ? activateSessionTab(current, rootTab.id)
            : openSessionTab(current, rootId);
        }
        if (isRestore && current.tabs.length > 0) return current;
  ...
  const clearClosedDraftTabs = useCallback(
    (closedTabs: readonly SessionTab[]) => {
      for (const closed of closedTabs) {
        ...
        if (activeNewSessionDraftKeyRef.current === draftKey) {
          ...
          setNewSessionDraftId((prev) =>
            prev === closed.id ? "initial" : prev,
          );
        }
      }
    },
  ...
  const handleTabCloseOthers = useCallback(
    (tabId: string) => {
      ...
      setTabState((current) =>
        closeOtherSessionTabs(activateSessionTab(current, tabId)),
      );
    },
    [sessionTabs, clearClosedDraftTabs],
  );
  ...
  const handleTabCloseRight = useCallback(
    (tabId: string) => {
      ...
      setTabState((current) =>
        closeRightSessionTabs(activateSessionTab(current, tabId)),
      );
    },
    [sessionTabs, clearClosedDraftTabs],
  );
    ...
    (tab: SessionTab) => {
      ...
      if (activeKey) {
        const owner = sessionTabs.find(
          (t) =>
            t.draftCwd !== undefined &&
            `new:${t.id}:${t.draftCwd}` === activeKey,
        );
        if (owner) {
          ...
          const activeDraftCwd =
            newSessionCwd ?? (selectedSession === null ? activeCwd : null);
          if (activeDraftCwd)
            rekeyDraft(activeKey, parkedNewSessionDraftKey(activeDraftCwd));
        }
      }
      ...
      rekeyDraft(
        getDraft(ownSlot) ? ownSlot : parkedNewSessionDraftKey(tab.draftCwd),
        draftKey,
      );
  ...
  useEffect(() => {
    ...
    // Viewing a subagent inside this tab's family is not a tab/session
    // mismatch — the subagent view lives in the family root's tab.
    if (
      selectedSession?.relation?.kind === "subagent" &&
      activeSessionFamily?.root.id === tab.sessionId
    )
      return;
  ...
  // welcome empty state takes over). Also forget the workspace's last-open
  // pointer so a reload doesn't resurrect the just-closed session.
  // tabsDrained remembers "the user closed every tab" so the activeCwd
  // fallback composer doesn't phantom-resurrect after a full drain — a fresh
  // boot (no tabs ever) still gets the fallback.
  const prevTabCountRef = useRef(0);
  const [tabsDrained, setTabsDrained] = useState(false);
  useEffect(() => {
    const hadTabs = prevTabCountRef.current > 0;
    prevTabCountRef.current = sessionTabs.length;
    if (sessionTabs.length > 0 || activeTabId) {
      if (tabsDrained) setTabsDrained(false);
      return;
    }
    if (!hadTabs) return;
    if (selectedSession) {
    ...
    setTabsDrained(true);
  }, [activeTabId, sessionTabs.length, selectedSession, tabsDrained]);
  ...
  const handleSessionDeleted = useCallback(
    (sessionId: string) => {
      ...
      if (selectedSession?.id === sessionId) {
        // No phantom composer: with tabs, the adjacent-tab restore effect picks
        // the next session; if this was the last tab, the welcome empty state
        // takes over (the tabs-drain effect clears the view).
        setSelectedSession(null);
        setSessionKey((k) => k + 1);
        setBranchTree([]);
        setBranchActiveLeafId(null);
  ...
  // After a full tab drain the fallback composer is suppressed (welcome page shows instead).
  const effectiveNewSessionCwd = tabsDrained
    ? null
    : (newSessionCwd ??
            ...
            <div style={{ flex: 1, overflow: "hidden", position: "relative" }}>
              {showChat ? (
                ...
                >
                  ...
                  >
                    {translate("workspace.welcomeHint")}
                  </div>
                  {sessionCatalog[0] && (
                    <button
                      type="button"
                      onClick={() => handleSelectSession(sessionCatalog[0])}
                      style={{
                        display: "flex",
                        alignItems: "center",
                        gap: 6,
                    ...
                    >
                      {translate("workspace.openRecent")}
                    </button>
                  )}
                </div>
              ) : null}
