...
// Compact single-line height of a project group header.
export const GROUP_HEADER_HEIGHT = 32;

/**
 * Windowing for the mixed-height project-group tree (group headers + session
 * rows) laid out at absolute `tops[i]` offsets. Only rows intersecting the
 * viewport (plus overscan) are returned, sorted ascending. A focused row
 * outside the window stays mounted so scrolling cannot discard an inline
 * rename.
 */
export function getSessionTreeIndices(
  tops: number[],
  heights: number[],
  scrollTop: number,
  ...
  const overscanPx = 8 * SESSION_LIST_ITEM_HEIGHT;
  const top = scrollTop - overscanPx;
  const bottom = scrollTop + (viewportHeight || 600) + overscanPx;
  const indices: number[] = [];
  for (let i = 0; i < tops.length; i++) {
    if (tops[i] < bottom && tops[i] + heights[i] > top) indices.push(i);
  }
  if (
    focusedIndex >= 0 &&
    focusedIndex < tops.length &&
    !indices.includes(focusedIndex)
  ) {
...
const EXPANDED_GROUPS_STORAGE_KEY = "pi-web:sidebar-expanded-groups-v1";
...
/** Persisted group expansion; null = no stored preference (default policy). */
function loadExpandedGroupKeys(): Set<string> | null {
...
function saveExpandedGroupKeys(keys: Set<string>): void {
  if (typeof window === "undefined") return;
  try {
...
export function SessionSidebar({
  ...
  const handleGroupNewSession = useCallback(
    (cwd: string) => {
      onNewSession?.(createTempSessionId(), cwd);
    },
    [onNewSession],
  );
  ...
  const toggleGroup = useCallback(
    (key: string) => {
      setExpandedGroupKeys((prev) => {
        // First explicit toggle materializes the default policy as stored state.
        const current =
          prev ??
          new Set(projectGroups.length > 0 ? [projectGroups[0].key] : []);
        const next = new Set(current);
        if (next.has(key)) next.delete(key);
        else next.add(key);
        saveExpandedGroupKeys(next);
        return next;
      });
    },
    [projectGroups],
  ...
  // (Activity counts are computed above the group tree.)
  ...
  // Mixed-height flat rows (group headers + their session rows) so the whole
  // tree shares one scroll container and one windowing pass.
  const treeRows = useMemo(() => {
    ...
    projectGroups.forEach((group, groupIndex) => {
      rows.push({ kind: "header", group, groupIndex });
      tops.push(offset);
      heights.push(GROUP_HEADER_HEIGHT);
      offset += GROUP_HEADER_HEIGHT;
      if (isGroupExpanded(group.key, groupIndex)) {
        for (const family of group.families) {
          rows.push({ kind: "session", family });
          tops.push(offset);
  ...
  const virtualIndices = getSessionTreeIndices(
    ...
    listScrollTop,
    listViewportH,
    treeRows.rows.findIndex(
      (row) =>
        row.kind === "session" && row.family.root.id === focusedSessionId,
    ),
  );
        ...
        >
          ...
          {projectGroups.length > 0 && (
            <div
              style={{
                position: "relative",
                height: treeRows.totalHeight,
              }}
            >
              {virtualIndices.map((index) => {
                const row = treeRows.rows[index];
                if (row.kind === "header") {
                  ...
                  return (
                    ...
                    >
                      ...
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                          handleGroupNewSession(row.group.root);
                        }}
                        title={t("sidebar.newSessionTitle", {
                          path: row.group.root,
                        })}
                        aria-label={t("sidebar.newSessionTitle", {
                          path: row.group.root,
                        })}
                        style={{
                          display: "flex",
